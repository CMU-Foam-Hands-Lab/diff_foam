...
class AutoHandNode(Node):
    ...
    def _publish_state(self, event=None):
        state = JointState()
        state.header.stamp = (int(self.get_clock().now().nanoseconds/(1e9)))
        # state.name = [""]
...
def main(args=None):
    ...
    rclpy.timer.Timer(autohand_node._publish_state, 0.2)
    rclpy.timer.Timer(autohand_node._write, 0.2)
    rclpy.timer.Timer(autohand_node._read, 0.5)
    while not rclpy.is_shutdown(autohand_node):
