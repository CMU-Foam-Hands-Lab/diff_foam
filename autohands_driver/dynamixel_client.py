...
class DynamixelClient:
    ...
    def __init__(self,
        ...
        self._pos_sync_reader = DynamixelPosSyncReader(
            self,
            self.motor_ids,
            pos_scale=pos_scale if pos_scale is not None else DEFAULT_POS_SCALE,
            vel_scale=vel_scale if vel_scale is not None else DEFAULT_VEL_SCALE,
    ...
    def connect(self):
        ...
        if self.port_handler.openPort():
            rclpy.logging.get_logger('dynamixel_client').info(f'Succeeded to open port: {self.port_name}')
        else:
            raise OSError(
    ...
    def read_sync_pos(self, retries: int = 2) -> np.ndarray:
        """Returns the current positions and velocities."""
        return self._pos_sync_reader.read(retries)
...
class DynamixelReader:
    ...
    def read(self, retries: int = 1):
        ...
        if not success:
            print("Failed to read bulk data")
...
class DynamixelSyncReader:
    ...
    def read(self, retries: int = 1):
        ...
        success = False
        while not success and retries >= 0:
            comm_result = self.operation

            comm_result = comm_result.txRxPacket()
        ...
        # If we failed, send a copy of the previous data.
        if not success:
            print("Failed to read sync data")
            return self._get_data()
...
class DynamixelPosSyncReader(DynamixelSyncReader):
    """Reads positions and velocities."""
